# CodeBuddy Desktop 1.1.0

1.1.0 将工作流、团队/子代理状态和 `/goal` 目标进度从聊天顶部移到可持续查看的右侧面板，并强化流式取消与消息渲染安全边界。

## 1.1.3（R12 加固 + R13 发版准备迭代）

1.1.3 包含 R12 加固迭代（PR #2）与 R13 发版准备迭代的全部内容。

### R13 发版准备（本轮新增）

- **构建链依赖安全升级（npm audit 14 → 4）**：electron-builder 24.13.3 → 26.15.3（消除 tar critical 与 app-builder-lib / builder-util / builder-util-runtime / electron-publish 等 8 项 high 告警；Linux `build:dir` + 测试门禁验证通过，Windows 全量打包由 release workflow 验证）；monaco-editor 0.55.1 → 0.56.0（ESM 入口重组，`esm/vs/...` 深路径迁移到新 exports 入口）+ 根 `overrides` 钉 dompurify ^3.4.14（消除 dompurify XSS 系列与 monaco 告警）。剩余 4 项均需 Electron / Vite 主版本升级（运行时/开发期大改，单独开轮，见迭代计划 Next）。
- **CLI 推荐版本 2.135.0 → 2.138.0**：`electron/cli-compat.cjs` RECOMMENDED_VERSION 提升（最低支持版本维持 2.125.0 不变；高于推荐版本仍仅警告不阻断）；README / CODEBUDDY 文档与设置页、CLI 引导对话框的 UI 回退值全量同步，`cli-compat.test.js` 新增版本钉住 + 文档同步守卫。
- **范围内补丁更新**：ws 8.21.0 → 8.21.3、eslint 9.39.4 → 9.39.5、prettier 3.9.4 → 3.9.6、autoprefixer 10.5.2 → 10.5.4（均为 semver 范围内 patch；vite 已在 5.x 最新补丁 5.4.21）。
- **R11 拆分首步**：`sessions-chat.js` 的 prompt 内容块构造纯函数抽取为 `src/lib/prompt-content.js`（行为保持不变，新增单测覆盖）。
- **代码签名文档补全**：`docs/release-checklist.md` 新增 Windows 证书导出 → base64 → GitHub secrets → 重跑 release workflow 的逐步操作指引。

### R12 加固迭代（PR #2）

- **合并/PR 门禁 CI 上线**：`.github/workflows/ci.yml` 在每次 push 到 master 与每个 PR 上跑 `npm ci` + `npm run test:gate`（ubuntu-latest + windows-latest 矩阵）；第三方 action 钉死 commit SHA（供应链加固）。
- **IPC 信任边界第二轮加固**：所有 `ipcMain.on` 通道（窗口控制、退出协商、rightBrowser 边界、productState:saveSync 等 15 个）统一 `requireTrustedMainSenderOn` 守卫（此前仅 `ipcMain.handle` 有统一守卫）；sender 校验新增 `senderFrame` 维度——iframe（子 frame）发起的特权 IPC 一律拒绝，frame 必须是主窗口自己的主 frame；契约测试扩展防回归。
- **npm 供应链治理**：`apps/mobile-remote`（Expo App）移出根 npm workspaces，桌面端 `npm ci` / CI 不再安装 Expo 依赖树；配合非破坏性 `npm audit fix`，audit 告警 42 → 14（剩余均需 electron-builder/Electron/Vite 破坏性 major 升级，已记录到迭代计划 Next）。
- **版本一致性守卫收紧**：RELEASE_NOTES 检查改为必须存在当前版本的章节标题行（原 `includes()` 可被任意段落中的版本号骗过）。
- **relay 离线行为钉死**：中继在 host 控制 socket 离线时对客户端的反馈是 `close(4001 'server offline')`；半开窗口内的客户端帧按设计丢弃（注入明文通知帧会破坏 E2EE 帧解析协议），新增注释与测试锁定该行为。

## 1.1.2（R1–R10 修复迭代发版）

1.1.2 是 1.1.x 系列第一个实际发布安装包的版本。此前 `v1.1.0` / `v1.1.1` 标签指向的提交**不包含** R1–R10 修复迭代（也未发布安装包），`v1.1.2` 标签是首个完整包含以下内容的可发布提交：

- **R1–R10 全部修复**（详见下方「1.1.x 修复迭代（R1–R10，PR #1）」小节）：应用内更新白名单断链修复（P0）、PTY 静默终端丢输出修复（P0）、mobile-remote prompt 对齐 ACP 契约、长任务不再被 120s 截断、relay 内存/丢帧修复、跨平台测试门禁、IPC 信任边界统一、lint 收紧至零告警。
- **发版自动化上线**：`v*` 标签触发 GitHub Actions `release.yml`（windows-latest → 测试门禁 → 构建 → draft Release 附安装包/blockmap/latest.yml/SHA256SUMS）。本版本安装包由该管线产出；若仓库未配置代码签名证书 secrets，产物为未签名预览版，Windows SmartScreen 可能提示「未知发布者」，需「仍要运行」。

## 1.1.1（对抗性审查修复）

1.1.1 依据生产交付流程的对抗性审查，逐项核销 1.1.0 的审查缺口：安全、可访问性、发布管线与实机 QA 覆盖。

### 安全（mobile-remote H14）

- 恢复被误删的 mobile-remote 客户端与协议/加密/中继包（无授权删除导致 `test:gate` 与 pre-push 失败，发布管线中断）。
- E2EE 握手签名补全：host 用 relay-auth Ed25519 密钥签名握手原文（serverId + 客户端临时公钥 + 时间戳），客户端验签后才信任解密载荷，封堵主动 MITM 换钥（08-06 审查 4 项协议缺陷的最后一项）。
- relay 传输 TLS 默认开启（`relayUseTls: true`），明文 ws 改为可显式关闭的例外。

### 可访问性与对比度（M4 实测）

- 亮色主题 muted/tertiary `#8e8e93` → `#707070`（2.99:1 → 4.55:1），暗色 `#7c7c82` → `#848484`（4.10:1 → 4.55:1），均按 WCAG 公式在真实面板底色上实测。
- 面板内 accent 改为面板作用域 token：亮色刷新按钮 `#1677ff` → `#0f63d8`（3.77:1 → 5.08:1）；暗色提交按钮底色 `#60a5fa` → `#1f6feb`（白字 2.54:1 → 4.63:1）。
- 目标/任务行补 sr-only 状态文本；目标行 key 稳定化（sequence/eventKey）；移除旧 WorkflowRightPanel/WorkflowStatusPanel 的全部死 CSS。

### 代码卫生

- `firstValue`/`normalizeStatus`/`normalizeMember` 单源化（新 `src/lib/workflow-normalize.js`）+ electron 跨进程一致性单测（防漂移）；`usePanelT` 共享 hook；`present*` 布尔函数改名 `should*`；`mergeMembers` 抽具名 helper；6 个 manual-*gui 脚本复用 e2e-driver 的 `wait`。
- Canvas 占位路由移除（无真实后端，符合 CODEBUDDY.md 路由纪律）；CODEBUDDY.md 同步修正（Docs 恢复为真实功能）。
- `normalizeWorkflowStatus` 引用级记忆化（有界缓存）；`closeWorkflowPanel` 不再回退 `activePromptRunId`（契约 §1）。

### 实机 QA 覆盖（M6 终门）

- panel-qa 扩展至约 46 项：键盘焦点全流程（初始焦点/面板内 Tab 环/Escape 焦点返回）、IME 组合态守卫（CDP 真实组合）、双主题对比度 computed-style 实测、亮暗即时切换、reduced-motion、窄窗自适应、高频流式渲染计数（DOM 变更不随 chunk 线性增长）、超长 message（4096 上限）、连点守卫、刷新竞态 latest-wins、behind-only/detached HEAD/空仓库/GBK 引号转义安全、跨项目与删除线程生命周期关闭、dismiss 动画期幂等、跨回合串扰清除。
- 新增像素三方 diff 工具（实机面板 vs 改动前基线 vs 原型 v3，1px 容差 + 超差理由日志），输出 `.omo/evidence/panel-pixel-diff/`。
- 新增原型 v3（`docs/prototypes/workflow-panel-v3.html`，计划 P0-1 的 10 项必含状态）。

### 安全加固与体验（本批新增）

- **主进程工作区信任模型**（`electron/workspace-trust.cjs`）：信任目录 = 对话框选择的目录（10 分钟 TTL）∪ 磁盘产品状态中的 `projectsById.*.workspacePath` ∪ `preferences.workspaceExtraDirs`；运行时项目 cwd、MCP cwd 与 git 允许列表在发起时都在主进程重新核验，渲染层无法伪造工作区路径。
- **IPC 发送者信任**：所有特权 IPC handler 统一经过 `requireTrustedMainSender`（`event.sender` 必须是主窗口），封堵伪造 sender 的调用面；`git-security.cjs` 提供与可信 git 检查复用的发送者信任助手。
- **窗口状态持久化**（`electron/window-state.cjs`）：重启后恢复上次窗口位置/尺寸/最大化，带工作区重叠面积校验与最小尺寸钳制，跨屏显示器变化不会把窗口恢复到屏幕外。
- **文件编辑工具白名单**（`src/lib/file-edit-tools.js`）：工具名统一归一化（camelCase→snake）后匹配 `write`/`edit`/`str_replace`/`notebook_edit`/`apply_patch`，挂进 ACP 工具门禁。
- **会话用量统计持久化** + **ACP 请求模式分发**：`resolveAcpRequestMode` 区分 live / usage-refresh / history-replay / rebind，仅用量类请求不再走流式渲染路径。
- **mobile-remote 设备管理加固**：设备存储支持 admin 设备，配对手牌加 TTL 与一次性 token，吊销仅限 admin 或设备自身。
- **工作流面板 git 分支菜单**：面板内可查看当前分支、打开变更、切换/新建分支，复用单源的 normalized 工作流状态。

### 1.1.x 修复迭代（R1–R10，PR #1）

- **应用内更新断链修复（P0）**：更新下载白名单接受仓库更名后的 `codebuddy-desktop`（保留 `codebuddy-gui` 兼容历史资产），并修复原白名单正则不匹配时放行任意 github.com 地址的缺陷（`electron/update-urls.cjs`）。
- **PTY 静默终端不再丢输出（P0）**：PTY SSE 输出流补 `timeoutMs: 0`，不再被主进程默认 30s chunk 空闲超时误杀。
- **mobile-remote prompt 对齐 ACP 契约**：`session/prompt` 改发内容块数组（与桌面端同形）；完成判定改为 JSON-RPC 响应 + `stopReason`（`result.done` 仅旧版兜底）；`prompt_done` 携带 `stopReason` 且保证只发一次。
- **mobile-remote 长任务不再被 120s 截断**：prompt 流超时从 wall-clock 改为 per-chunk 空闲窗口（与主进程 openStream 同款 armTimeout 模式）。
- **relay 内存与丢帧修复**：pending 帧补 1 MiB 字节上限（条数上限之外）；host data-socket 断开期间 client 帧改回缓冲并在重连后 flush，不再静默丢弃。
- **跨平台测试门禁**：Windows 形状用例 `runIf` 化、构建产物断言默认跳过（`CODEBUDDY_REQUIRE_BUILD=1` 强制），`test:gate` 在 Linux 无构建产物可全绿。
- **IPC 信任边界统一**：全部 `ipcMain.handle` 通道要求可信主窗口 sender；`codebuddy:request` renderer 销毁改为真实 abort。
- **Lint 收紧**：`no-undef` 打开、warn → error，全量零告警（顺手修复 1 处测试缺 `vi` import）。
- **发版自动化**：新增 GitHub Actions `release` workflow（v* tag → windows-latest → 测试门禁 → 构建 → draft Release 附安装包/blockmap/latest.yml/SHA256SUMS；无签名证书 secrets 时明确出未签名预览）；维护者手册 `docs/release-checklist.md`。

## 本版本重点

### 输入、终端与切换性能

- **打字不再触发全量状态落盘**：草稿/时间线持久化防抖提升到 1500ms，渲染进程持久化按「合并节流」执行（连发的落盘触发收敛为一次写入）；主进程写入改异步、去 pretty-print 并增加 800ms 合并窗口，写盘不再阻塞按键等 IPC。
- **打字不再扇出全局重渲染**：输入框草稿改为组件本地状态，打字不触碰全局 store；失焦/发送/切换会话时同步回 store（草稿不丢）。顶栏与工作流面板改为标量订阅，会话树按行 memo 化。
- **全局快捷键只解析一次**：快捷键绑定模块级缓存，按键热路径不再每次读 localStorage。
- **流式与终端批处理**：SSE 流消息在主进程按 33ms 窗口批量发送（终止事件立即 flush）；终端输出在渲染侧按 50ms 合并写 store（xterm 显示仍实时）；终端落盘防抖 500ms→5s，快照在写盘时重新捕获；终端按键输入 16ms 攒批后一次发送。
- **切换视图不再重建**：已访问视图保持挂载（非活跃 `display:none`），聊天长文、xterm、编辑器往返切换不再全量重建；切换项目时自动清空保活缓存防止跨项目串数据。隐藏视图的无状态轮询（文件 watcher、日志、实例、指标等）自动暂停。
- **切换不再等待落盘**：切换会话路径上的持久化改为后台执行，失败不阻断切换；右侧浏览器面板的逐帧 IPC 改为事件驱动 + 100ms 节流。
- **前台流式 chunk 合并（已知限制，暂不实施）**：流式文本 chunk 频率低（1-5/s），50ms 合并窗口内通常只有一个 chunk，无实际收益；IPC 侧风暴已由主进程 33ms 批量发送解决，终端类高频输出已有独立的 50ms 合并。若实施需改造 20+ 处依赖「chunk 同步应用」的测试契约，收益与成本不匹配，故明确降级为已知限制（代码注释 `product-persist.js queueCoalescedTimelineEvent` 有完整记录）。

### 工作流与目标状态

- 新增右侧工作流面板，集中展示当前阶段、整体进度、耗时、tokens、工具调用、团队成员和子代理任务。
- 工作流进度改为读取 CLI 持久化的 workflow record / journal，并用运行时登记的可信项目目录定位记录；prompt 线程先进入 idle 时，仍持续投影真实阶段与子代理状态。
- 工作流结束后开启有上限的后台响应接收窗口，锁定首个 server requestId，只接收同一请求的最终汇总；匹配的 `session_end` 会关闭流式消息并写入完成时间，过期或无关请求不会串入当前对话。
- 实机后台汇总验证已经落到 `TESTING.md` 的专项门禁，覆盖真实进度读取、prompt idle 后续收、requestId 归属隔离和最终正文收口。
- `/goal` 支持当前目标看板与可折叠更新记录；目标进度可从实时事件和已保存的会话时间线恢复。
- 工作流首次产生活动时自动打开面板；用户手动关闭后，同一轮运行不会反复打扰，完成、失败或取消后仍可重新查看最终快照。
- 成员消息保留在对应的成员历史中，聊天正文隐藏重复的工作流 bookkeeping；窄窗口下右侧面板以抽屉形式显示。

### 聊天可靠性与安全渲染

- 新增**生产性能门禁**（`npm run test:perf:production` + `test:perf:memory`，已接入 `test:release`）：
  - 固定 300 条 transcript fixture（≥100 条 Markdown、≥50 条 fenced code block 覆盖 js/json/python/shell、20 条约 200KB 边界消息），fixture 通过真实 `reduceAcpEvent` reducer 水合，测试脚本禁止 `patchThreadRuntime({ timeline })` 重建；
  - 输入延迟使用真实 `Input.dispatchKeyEvent` 按键路径测量（空 transcript p95 ≤ 35ms、max ≤ 100ms；300-entry 及流式期间 p95 ≤ 50ms），10 秒窗口 long task >100ms 为 0、>50ms 不超过 2 次；
  - 300-entry chat 首次可交互 median ≤ 1.5s；terminal/editor/settings 返回 chat p95 ≤ 150ms；
  - heap slope ≤ 1 MiB/轮、全路由 retained 增量 ≤ 80 MiB、DOM nodes ≤ 基线 × 1.25、jsEventListeners 增量 ≤ 100 且无连续 3 轮单调增长（基线由首轮 collect-only 确立并人工审查）；
  - bundle 绝对预算之上增加按 label 的已提交历史基线增长门禁（raw >10% 且 >50KB / gzip >10% 且 >10KB 才失败），基线只能显式 `test:bundle-budget:update` 更新。
- **消息 Markdown 渲染上限**：remark-gfm 对长内容存在病态解析（实测 ~5.6KB/s，171KB 表格密集消息会阻塞主线程约 30 秒），单条消息 Markdown 渲染上限从 200KB 收紧到 4KB，超限消息按纯文本展示（「消息内容较长，已按纯文本显示」），保证大 transcript 首屏与流式期间不再冻结。
- 点击 Stop 先立即结束本地流和线程运行态，后端取消通知异步发送；正常用户取消不再显示红色发送失败，迟到状态也不会覆盖 `cancelled` 终态。
- assistant Markdown 禁止执行原始 HTML；外部链接交给隔离的右侧浏览器面板，危险协议、凭据 URL、超长或异常消息按单条消息安全降级。
- 右侧文件浏览器复用真实工作区文件状态和操作，浏览器、文件和工作流面板默认收起；中文和英文思考强度均保留 `ultracode` 原始小写名称。

### 可靠性与验证

- 统一处理团队、工作流、目标和子代理事件，支持重复事件去重、乱序进度保护以及迟到事件的会话归属校验。
- 新增 Stop、Goal 聚合、Markdown 安全渲染、工作流状态和右侧面板回归测试；后台 workflow 汇总门禁覆盖真实进度读取、prompt idle 后继续接收、请求隔离、匹配 `session_end` 收口和最终正文完整渲染。
- 测试命令和当前 Windows 环境限制见 [`TESTING.md`](./TESTING.md)；完整发布门禁不因环境错误被伪造为通过。

### 传输失败快速重连（无自动重发）

- **真网络断开**（IPC 断流、连接拒绝等）秒级进入有限退避重连（1s→30s，最多 10 次），成功后自动 `initialize` + `session/load` 绑定旧会话；失败进入 `error` 终态，不再永久卡在「重连中」。
- **不自动重发用户消息**：无后端幂等键时禁止二次 `session/prompt`，失败走历史恢复 / 错误卡 / 草稿恢复，避免重复执行工具副作用。
- **错误分类**：HTTP 401 走登录引导；403/4xx/429 与上游 5xx 不拆连接；长任务 idle 超时视为会话可恢复，不拆连接。
- **delayed rebind**：重连时若有进行中的 turn，先只恢复协议；turn 结束后再补 `session/load` 并清除 `sessionRestoreNeeded`，历史重放事件不会污染已终态时间线。
- **kill switch**：`guiSettings.transportAutoReconnect`（默认 true）；设为 `false` 时只标记断连、不调度自动重连。
- 新增回归：`tests/unit/acp-auto-reconnect.test.js`、`acp-error-classify.test.js`、`prompt-transport-recovery.test.js`。


1.0.5 对照真实 CodeBuddy WebUI（CLI 2.125 `dist/web-ui`）逐功能修复差异：侧栏导航与各页标题对齐、附件阈值/数量上限对齐、新增 max_tokens 截断态、插件市场浏览、会话搜索、免打扰、会话级权限行为项、Keybindings 上下文文案、关于页等。

## 本版本重点

### 侧栏与页面对齐真实 WebUI

- 新增「无限画布」路由与占位页（标题/工具栏/空态文案对齐真实 `canvas.title`）。
- 侧栏导航项标签对齐真实 WebUI：实例→实例列表、编辑器→代码编辑器、链路→链路追踪、指标→系统监控、日志→日志查看器、Workers→Agent 实例管理、文档→文档中心。
- 各页面标题对齐：Workers→Agent 实例管理、Instances→实例列表、Logs→日志查看器。
- 任务页主标题→定时任务、加副标题「当前会话中的活跃定时任务」、任务模板→推荐模版、刷新模板→刷新推荐、空态→暂无推荐模版；新增 6 个周期预设快捷按钮（每天/每2天/每3天/每周/每2周/每月）。

### 会话与附件

- 附件阈值对齐真实 WebUI：图片 20MB→10MB（文案「图片不能超过 10MB」）、文本文件 5MB→1MB（「文件不能超过 1MB」）；新增数量上限校验（最多 5 张图片 / 10 个文件）。
- max_tokens 截断态处理：stopReason=max_tokens 时追加黄色 notice 时间线卡片并以 idle 完成（非硬错误），与真实 `chat.error.maxTokens` 一致。
- 「无法删除当前会话」约束：删除按钮禁用 + confirmDelete 双重守卫。
- 会话搜索：在会话列表树顶部新增搜索框，跨项目模糊匹配会话标题/项目名（保留原有项目-会话树结构）。

### 设置与权限

- 新增「免打扰」设置项（doNotDisturb），开启后抑制所有后台任务桌面通知。
- 新增「本会话自动通过文件编辑权限」「每次首次使用工具时都请求权限」行为项（sessionAutoAllowFileEdits 实现文件编辑工具自动放行）。
- 新增「关于」设置区块：版本、许可证、服务条款、隐私政策；应用更新行新增「查看更新日志」按钮。
- Keybindings 9 条上下文触发文案对齐（聊天输入框/终端/设置/帮助/命令面板/插件对话框/权限对话框/自动补全/消息选择器）。

### 插件市场

- 接线 `browseMarketplace`：市场行新增「浏览」按钮 + 浏览 Modal（搜索/列表/已安装标记/一键安装）。

## 1.0.4

# CodeBuddy Desktop 1.0.4

1.0.4 修复多工具回合中 **迟到 SSE 最终正文** 被丢弃或误报「最终正文未送达」的问题；GitHub 仓库统一为 `codebuddy-desktop`，自述文档与产品名对齐 **CodeBuddy Desktop**。

## 本版本重点

### 流式回复可靠性

- GET SSE 上无 `requestId` 的 `agent_message_chunk`：在 prompt 活跃时改为入队；prompt 结束后短时内仍关联最近一次 `promptRunId`。
- 最终回复 grace 窗口延长至 1.5s，覆盖 RPC `end_turn` 后仍到达的摘要块。
- 回合内已有非空助手正文（含工具前叙述）时 **软成功**：保留已展示内容，不再强制历史恢复或硬错误横幅。
- 单测覆盖 late SSE 关联、pre-tool body 可用性与 soft-success 路径。

### 文档与仓库

- README 标题与链接改为 **CodeBuddy Desktop** / `ChisaAlter/codebuddy-desktop`。
- `package.json` homepage / repository 指向重命名后的 GitHub 仓库。

## 1.0.3

1.0.3 完成 **CLI 2.125 桌面能力接线**：附加工作目录、插件市场自动更新与 HTTP 更新、插件列表无限滚动、文件检查点预览与回退、AskUserQuestion 取消语义锁死，以及 Skills / Agents 只读面板。

### CLI 2.125 适配

- 附加工作目录（`workspace-dirs`）：composer 可选目录、项目偏好持久化，并与 CLI `POST/DELETE/sync` 对齐。
- 插件市场 `autoUpdate` 与 `POST /api/v1/plugins/update`：HTTP 优先更新，失败回落现有 CLI `plugin update`。
- 插件列表无限滚动、回到顶部与类型筛选（Skills / MCP / Hooks / Tools）。
- 文件变更检查点：Changes 面板支持 diff 预览与按路径 / 检查点回退（`/internal/file-changes/*`，diff 使用绝对路径）。
- AskUserQuestion 取消：跳过/取消走 `resolveInterruption(deny)` 或 `{ outcome: 'cancelled' }`，**不**调用整轮 `session/cancel`；单测与 live smoke 锁定。
- Skills / Agents 只读页：无公开列表 API 时降级为空态或插件元数据，不伪造可写成功。
- `fetchJson` 正确处理 **204 No Content**（workspace-dirs / auto-update 等 mutation）。

### 聊天与体验

- 聊天滚动辅助、时间线与取消语义相关单测扩展。
- 侧栏入口与路由接入 Skills / Agents 页面。

## 1.0.2

1.0.2 基于当时 master 的稳定性与品牌收口版本：修复登录循环与 502 误报，统一 CodeBuddy Desktop 品牌，提升 CLI 兼容版本至 2.125.0，并增强退出可靠性与侧栏/聊天可测性。

- 将网络/代理 502 与云端鉴权失败正确分类，避免发消息后被误判为需要重新登录。
- 解析 CLI JSON `errorMessage`，透出代理与模型端点详情，便于排查连接问题。
- 登录成功后软恢复磁盘 OAuth；账户鉴权边界与侧栏身份卡展示一并收紧。
- `product-state` 拒绝空项目列表覆盖有效状态；未 hydrate 时跳过 flush，防止状态被清空。
- 托盘完全退出缩短超时，并支持脏文件对话框 hold，避免未保存内容在退出链中丢失。
- 产品名与快捷方式统一为 **CodeBuddy Desktop**；最低 / 推荐 CLI **2.125.0**。
- 附件选择、侧栏新对话、自定义模型、模式高亮与运维视图可测性增强。

## 1.0.1

1.0.1 在桌面稳定性修复之上，补齐输入栏交互体验与 CodeBuddy 品牌图标，并继续收口新会话发送、长任务、权限卡和偏好持久化。

### 聊天输入栏

- 模式 / 模型 / 思考强度切换改为乐观更新：一点击立刻换文案并关闭菜单，不再出现整栏禁用的卡顿感；失败时自动回滚并提示。
- 选中值切换时带 3D 翻转入场动画；下拉菜单支持进出过渡与选项 hover 轻阴影。
- 点击选择控件时不再给输入框叠蓝色 focus 边框。
- 模型与思考强度收为一组贴近排列，与发送按钮拉开间距；窄窗口下截断文案而不是把思考强度挤没。
- 发送 / 停止按钮对齐 WebUI：圆形上箭头发送、圆形实心方块停止。
- 附件入口支持按图片 / 文件分类选择（主进程对话框过滤）。

### 品牌与图标

- 应用品牌图标替换为青色像素机器人头（窗口、托盘、通知、登录页、侧栏、安装包 `.ico`）。
- 品牌资产 SHA256 测试同步更新。

### 稳定性与偏好（延续）

- 修复新会话刚连接时线程记录尚未写入 sessionId，首条消息可能以 `null` 调用 `session/prompt` 并返回 `Invalid params` 的竞态。
- 长时间运行的 `session/prompt` 不再受 120 秒空闲超时限制；普通 ACP 请求仍保留 30 秒超时边界。
- 权限请求同时按 `interruptionId` 和 `toolCallId` 去重，旧会话的迟到事件不会再写入当前线程。
- GUI 主题、提示建议、桌面通知等偏好写入 `product-state.json`，切换项目或重启后会恢复，不再依赖动态本地端口对应的 localStorage。
- `Ctrl/Alt/Meta` 组合快捷键在聊天输入框、Monaco 和终端聚焦时仍可执行；无修饰键和仅 Shift 的绑定继续让位于正常文本输入。
- 侧栏折叠宽度固定为真实 60px，展开宽度保持 220px 到 252px；移除会被后台窗口节流的布局过渡，快捷键切换立即生效。
- 桌面验收夹具同步当前 19 个路由和运行时就绪日志，使用隔离产品状态与确定性控件触发，避免后台窗口、高 DPI 和旧选择器造成误报。

## 1.0.0

1.0.0 首个稳定版本，完成编辑器资源精简、静态质量收口和 Windows 本地发布链路整理。

- Monaco 改为核心编辑器、单一 editor worker 和实际使用语言的按需高亮，不再打包 TypeScript、CSS、HTML 和 JSON 语言服务 worker。
- 前端 assets 从约 14.85 MB 降至约 5.18 MB，文件数量从 113 个降至 32 个；工作区仍支持 JavaScript、TypeScript、JSON、Markdown、Python、CSS、HTML、YAML、Shell 和纯文本。
- JSON 使用轻量 tokenizer 保留注释、括号、引号自动闭合和语法着色，不引入完整 JSON language service。
- ESLint 正确识别 JSX 变量和项目既有 `_` 占位参数，清理真实死代码与异常空白，完整 lint 达到 0 error / 0 warning。
- GitHub Actions 继续保持关闭，Windows 安装包和更新清单由本地发布命令生成，不依赖 CI。

## 0.9.0

0.9.0 补齐 CodeBuddy CLI 指定版本安装与回滚。

- 设置页 CLI 维护区新增“安装版本”，支持 `latest`、完整版本号和带 `v` 前缀的版本输入；填写旧版本号即可执行回滚。
- 安装前使用独立应用内对话框确认目标，渲染层与主进程分别校验版本格式，不允许把任意文本拼入 CLI 命令。
- 安装调用真实 `codebuddy install <target>`，最长运行 10 分钟，并与诊断、更新和插件维护共用独占操作锁。
- 安装完成后重新读取 CLI 版本，显示安装前后版本和真实命令输出；输出过长时保留最新 1 MB。
- 无论版本是否变化，安装完成后都会提示重启当前项目运行时，避免继续使用安装前已启动的进程。

## 0.8.0

- 后台会话列表新增“接管”按钮，点击前重新读取 `codebuddy ps --json`，确认 PID 仍存在且类型为后台任务。
- 接管通过真实 `codebuddy attach <pid>` 完成，优先在 Windows Terminal 新标签页中打开；Windows Terminal 不可用时回退到独立 Windows PowerShell 窗口。
- 交互终端使用后台会话自己的工作目录；PID 和目录均在主进程重新校验，不接受渲染层直接拼接命令。
- 接管与启动、终止操作共用同步操作锁，避免快速重复点击；GUI 明确反馈终端类型、打开失败和“关闭窗口不会终止后台任务”。
- 交互会话、Daemon 和预热进程继续不显示接管入口，避免对不适用的会话调用 `attach`。

## 0.7.0

- 插件卡片新增“更新”入口，按插件真实安装作用域调用 `codebuddy plugin update`，执行前使用应用内确认。
- 插件页支持在用户全局、项目共享和项目本机三种作用域检查失效自动依赖；先运行 `--dry-run`，只有发现可清理项才允许确认删除。
- 插件更新和依赖清理均展示可复制的真实 CLI 输出，限制最大输出体积，并与 CLI 更新、诊断共用独占操作锁。
- 插件变更完成后明确提示当前运行时仍加载旧状态，可一键重启当前项目运行时并重新加载插件与市场列表。
- Windows 构建入口在 electron-builder 最终因 `rcedit`、`EBUSY` 或 `EPERM` 退出失败时等待 2 秒并仅重试一次；其他构建错误保持立即失败。

## 0.6.0

- 设置页新增 CodeBuddy CLI 维护区，直接显示当前 GUI 实际调用的已安装 CLI 版本。
- 可运行真实 `codebuddy doctor` 并查看、复制诊断输出；45 秒未完成会终止整个命令进程树并明确反馈超时。
- “检查并更新”在应用内确认后运行真实 `codebuddy update`，同一时间只允许一个诊断或更新操作，避免重复修改本机安装。
- 更新完成后重新读取版本；版本变化时明确说明现有项目进程仍使用旧 CLI，并提供当前项目运行时的一键重启入口。
- 诊断和更新输出均限制内存占用，过长时保留最新内容并在界面标记截断；更新最长运行 5 分钟。

## 0.5.0

- Workers 页新增 Daemon 登录自启动状态，直接读取 `codebuddy daemon status` 的系统服务信息。
- 支持配置可选端口和权限模式，并通过真实 `codebuddy daemon install` 注册用户级 Windows 任务计划。
- 卸载操作使用应用内确认，调用 `codebuddy daemon uninstall` 后重新读取状态确认任务计划已删除。
- 系统服务操作与当前项目运行时隔离；切换项目时迟到结果不会覆盖新页面，卸载自启动不会强制终止当前 Daemon。

## 0.4.0

- 实例页新增“项目运行时 / 后台会话”双视图，后台会话列表直接读取 `codebuddy ps --json` 的结构化结果。
- 可选择已添加项目并输入任务内容，通过真实 `codebuddy --bg` 启动独立后台任务；名称、工作目录和任务内容均在主进程重新校验。
- 后台任务支持日志读取、5 秒自动刷新、复制、最新 1 MB 截断提示、本机 Endpoint 打开和终止确认。
- 交互会话、Daemon 与预热进程可查看但不会显示终止入口，避免误杀 CodeBuddy GUI 自己维护的项目运行时。

## 0.3.0

- 工作区新增 Sandboxes 页面，读取 CodeBuddy 的真实 `sandbox-state.json`，展示当前 Sandbox、模板、别名、创建与最近使用时间。
- 展示每个 Sandbox 的本地到远端项目映射和最近同步时间，支持按 ID、别名、模板及路径搜索。
- 终止 Sandbox 和清理失效记录均调用真实 `codebuddy sandbox` 命令，操作后重新读取状态确认结果，并明确反馈缺少 `E2B_API_KEY` 等 CLI 错误。

## 0.2.0

- 工作区新增 MCP 页面，统一查看当前项目本机、项目共享和用户全局三种作用域的服务器配置与真实配置文件位置。
- 支持添加 stdio、HTTP 和 SSE 服务器，删除前使用应用内确认，并在写入后重新读取配置确认真实结果。
- 已配置服务器会读取当前项目运行时状态，并可查看真实工具列表；切换项目时旧状态和迟到响应不会覆盖新项目页面。

## 完整功能

- MCP 配置文件按 CodeBuddy 的 JSONC 路径规则读取，界面只展示环境变量和请求头的键名，不返回其中的凭据值。
- MCP 写操作绑定发起时的项目和运行时代次；运行时未连接时仍可查看配置，但禁用添加、删除和工具读取。
- GUI 更新下载仅接受本仓库正式 Release 下版本号与文件名一致的 `CodeBuddy-GUI-Setup-<version>.exe`，安装仍由用户主动确认。
- Windows 窗口最小尺寸从 1200×760 调整为 900×640，支持小屏、远程桌面和左右分屏。
- 展开侧边栏会在 220px 到 252px 之间响应式收缩，折叠宽度保持 60px。
- 工作区文件树和 Git 变更列表改为带下限的响应式宽度，为编辑器和 diff 保留更多空间。
- 工作区侧栏在 280px 到 360px 之间随窗口变化，变更列表在 300px 到 380px 之间变化；现有横向滚动和主区域 `min-w-0` 行为继续保留。
- 默认窗口仍为 1440×920，已有窗口位置和最大化状态继续恢复；仅放宽用户主动缩小窗口时的下限。
- 每个终端面板独立维护帮助弹层状态；弹层打开时保留原面板焦点行为，不会误切换或操作终端会话。
- 原有悬停查看继续保留，键盘聚焦时也会显示帮助内容；图标对辅助技术隐藏，按钮提供明确名称。
- 渲染入口统一序列化 Error、字符串和对象形式的拒绝原因，并监听捕获阶段的资源错误及全局 `unhandledrejection`。
- 同类型、同消息和同堆栈的错误在 2 秒内只上报一次，去重记录会自动清理；主进程继续限制字段长度并在落盘前脱敏。
- 每个会话拥有独立的 350ms 草稿保存计时器；切换输入会刷新本会话计时器，不会影响其他会话的草稿状态。
- 会话发送、状态更新等即时持久化路径会吸收待处理草稿保存，避免随后重复写入；最终退出保存继续覆盖全部当前草稿。
- 托盘“完全退出”现在先处理未保存编辑内容，再保存当前工作区并执行最终同步快照；只有快照确认成功才通知主进程退出。
- 退出保存链发生异常、同步 IPC 不可用或主进程拒绝写入时，应用保持打开并显示失败原因，不再静默丢失最后状态。
- 渲染错误边界会向主进程报告错误消息、堆栈、组件堆栈和当前路由；日志写入前会脱敏并在超过 1MB 时保留尾部，避免长期增长。
- 渲染进程异常退出会记录 Electron 提供的原因和退出码，并由主进程提供恢复对话框；React 崩溃页可直接打开用户数据目录获取 `crash.log`，打开失败时保留明确反馈。
- 聊天输入区可直接拖放本地文本文件和图片；读取期间会锁定重复操作，失败原因会显示在原会话，拖放结果不会因项目或会话切换写入错误页面。
- 设置页复制用户数据目录等系统路径时使用统一剪贴板兼容回退，避免安装版或受限环境下 Clipboard API 不可用导致无响应。
- 设置页可一键导出 JSON 诊断报告，包含应用、系统、窗口、运行时状态和最近启动/崩溃日志；密码、Token、API Key、用户目录和项目路径会脱敏，不读取对话、草稿、项目文件或 `product-state.json`。
- 设置页提供 CodeBuddy GUI 手动更新检查：显示当前与最新版本、区分尚无正式发布与网络失败，15 秒超时后可重试；可直接打开本仓库对应版本的 Windows 安装包下载地址，不会后台静默下载或自动安装。
- 窗口隐藏或未聚焦时，CodeBuddy 后台任务完成或失败会发送桌面通知；待发送队列只在最终结果时通知，用户取消不误报失败，点击通知会恢复窗口并打开对应对话，设置页可随时关闭此功能。
- CodeBuddy GUI 的主题、提示建议、剪贴板贴图和 Token 计数现在独立保存在本机，不再写入 CodeBuddy CLI 项目设置；旧版本共用缓存会自动迁移，切换项目不会重置这些偏好。

- 首次关闭窗口到托盘时会明确提示应用仍在后台运行，提示状态随窗口配置持久化；托盘菜单使用“打开 CodeBuddy GUI”和“完全退出”区分隐藏与真正退出。
- 聊天消息和 Worker 日志复制支持 Clipboard API 与兼容回退，成功和失败都会在原按钮上反馈，并在页面卸载时清理状态计时器。
- 设置页可通过主进程直接打开安装版用户数据目录，便于获取诊断日志和本地状态；应用信息加载、路径复制和目录打开均提供明确成功或失败反馈。
- 当前项目要求登录时，登录页会显示项目身份，并允许切换已添加项目、打开其他项目或重启当前运行时；项目与运行时先完成切换，再由新项目重新执行认证和会话初始化。
- 服务状态确认失败时可直接重启当前项目运行时并重试，或打开其他项目继续工作；工作区选择会区分成功、失败和用户取消，不再把恢复页变成只能重复重试的死路。
- 快捷键页明确区分 CodeBuddy GUI 与 CodeBuddy CLI：GUI 快捷键保存在本机并即时驱动侧边栏、新建对话及主要页面导航，支持按键捕获、重复与系统保留组合键校验和独立恢复默认；CLI 快捷键继续绑定当前项目运行时并保留原有组合序列语义。
- 多项目和多对话持久化，支持并行 CodeBuddy 运行时与后台会话。
- 文件浏览、Monaco 编辑器、项目终端和 Git 工作流。
- 插件、Worker、任务、实例、日志、指标、链路和远程渠道管理。
- Windows 托盘、单实例唤回、窗口状态恢复和可选安装目录。
- 次级页面按需加载，编辑器仅在进入工作区后加载。
- 首次启动时若 CodeBuddy CLI 不可用或项目目录已移动，会显示可操作的明确错误信息。
- 应用界面、Windows 产物和 ACP 客户端上报统一使用同一份产品名称与版本。
- 使用统计适配当前 CodeBuddy 数据契约，可查看会话、消息、活跃天数、模型 Token、常用工具和近期活动。
- 远程渠道仅显示当前 CLI 支持的操作；微信解绑后可直接重新扫码绑定。
- 系统监控展示当前 CLI 的实例健康、PID、目录、版本、运行时长和内存占用。
- 插件页兼容单个插件响应，并展示安装范围、作者、许可、技能数量和市场信息。
- Worker 页展示当前实例、Endpoint、模式和心跳，并避免误终止当前项目运行时。
- 快捷键管理支持搜索、上下文筛选、用户覆盖、后端校验、恢复单项默认和全部重置。
- 设置页补齐 Token 计数、通知、信任、状态栏和可信目录，并正确保存与回滚嵌套设置。
- 设置页明确区分 CodeBuddy GUI 与 CodeBuddy CLI 配置，移除误导性的 GUI 语言选项，并在项目设置重载期间保持主题稳定。
- 项目重命名和移除改为应用内确认流程，明确提示未保存修改，并在状态持久化失败时恢复操作前状态。
- Git 变更页使用应用内分支选择、新建分支和丢弃确认流程，并在操作失败时保留上下文与错误信息。
- 插件卸载和市场删除使用应用内确认与失败反馈，并阻止多个插件写操作同时执行导致状态互相覆盖。
- 远程渠道解绑和删除使用应用内确认与失败保留，删除微信实例时会同步停止对应二维码轮询。
- 定时任务删除、Worker 终止和快捷键全量重置使用统一应用内确认，并在失败时保留操作上下文。
- 文件重命名和删除改为应用内确认，目录操作会同步映射或关闭其中已打开文件，并明确保护未保存内容。
- 切换项目、工作区、目录或文件时，未保存编辑内容统一使用应用内确认，不再弹出系统原生确认框。
- 从系统托盘真正退出应用时会检查当前编辑器的未保存内容，取消确认即可继续留在应用中。
- 终端页会明确展示项目运行时停止或启动失败状态，并支持原地重新启动；不可执行的终端关闭操作会正确禁用。
- 对话页在运行时或会话连接失败时直接展示错误与恢复入口，未连接期间保留草稿但阻止无效发送。
- 消息发送失败时会恢复原草稿和附件，队列消息也由统一发送事务负责清空与失败回滚。
- 会话删除使用统一应用内确认，删除失败时保留弹窗与错误以便重试，并仅在后端删除成功后释放本地连接。
- Git 变更页按工作区和文件隔离异步结果，快速切换项目或文件不会被旧请求覆盖；提交和暂存按钮会匹配真实索引状态。
- Git 变更页可预览未跟踪文本文件，并对空文件、二进制内容和超长文件提供明确边界提示。
- 编辑器保存请求按项目、文件和请求序号隔离，保存期间切换文件不会再污染新文件的保存基线，保存结果会在编辑器内明确反馈。
- 工作区会监听磁盘文件变化并自动刷新未修改文件；存在本地编辑时会保留当前内容，并提供重新载入或继续编辑的明确选择。
- Electron 请求代理会保留图片二进制响应并提供标准 Headers、Blob 和 ArrayBuffer 读取能力，微信二维码可在安装版中正确显示。
- 远程渠道的加载、创建和二维码轮询按项目隔离；切换项目会清空企微凭据输入，旧项目请求不会再覆盖当前页面。
- 同一项目的运行时启动、停止和重启会串行执行，认证请求固定绑定对应运行时地址，项目切换期间不会串用密码或 Token。
- 项目运行时页支持不同项目并行操作、独立成功与失败反馈、后台状态对齐和持续更新的运行时长，旧刷新结果不会覆盖新状态。
- 从项目运行时页启动或重启当前项目时会重新连接当前会话并刷新运行时数据；停止后相关会话显示为已断开，持久化的待发送队列在重连时会保留并继续处理。
- 设置加载会区分系统信息与设置请求的真实失败，并提供明确重试反馈；切换项目会清空旧项目的设置视图。
- 设置写入固定绑定发起操作时的项目地址与认证上下文，模型和模式切换也只更新原会话，避免快速切换项目或会话时互相覆盖。
- 日志页切换项目时会重新加载 Worker，并按项目、Worker 和请求序号隔离日志结果；自动刷新产生的迟到响应不会覆盖当前选择。
- 插件安装、卸载、启停和市场增删只会收尾发起操作的项目；插件与市场错误分开呈现，不再互相覆盖。
- 定时任务创建、删除和刷新绑定原会话，切换会话会清理旧操作反馈，迟到结果不会修改新会话表单或错误状态。
- Worker 与 Daemon 的刷新、启停、重启和终止反馈按项目隔离，旧项目操作完成后不会刷新或关闭新项目的页面状态。
- 快捷键校验、保存和全量重置绑定发起操作的项目；校验期间切换项目会停止后续写入，旧请求不会修改新项目页面。
- Git 暂存、丢弃、分支、拉取、推送和提交操作固定绑定点击时的仓库目录，多步丢弃过程中切换项目也不会把后续命令执行到其他仓库。
- 切换分支、Pull 和丢弃修改会先保护编辑器未保存内容，成功后重新载入或关闭受影响文件；新分支首次 Push 会自动建立 origin upstream，Diff 视图也会跟随 GUI 主题。
- PTY 创建和释放固定绑定原项目运行时与认证上下文，切换项目后的迟到会话会被清理，旧终端输出、状态和重连结果不会写入新项目。
- 会话重命名和删除固定绑定发起操作的项目，旧项目操作完成后不会关闭或覆盖新项目的侧边栏状态。
- 侧边栏模型与模式切换按项目和会话隔离并显示结果；内联重命名会阻止回车与失焦造成的重复提交。
- 新建会话会统一锁定侧边栏和标题栏入口，避免快速点击创建多个空会话；状态保存失败会回滚，切换项目期间不会初始化错误的会话。
- 模型和模式写入按会话串行执行，聊天页会显示切换进度与真实结果，失败时保留选择菜单供用户重试。
- 权限响应、问题回答和停止生成固定绑定原会话并阻止重复请求；处理后的卡片会持久显示结果，失败时保留内容供重试，问题卡支持后端选项。
- 附件选择结果绑定原会话并自动去重；文件改为逐项异步读取，单个无效或超限文件不再拖垮整批选择，并补齐常见源码与配置格式。
- 项目、会话和工作区切换采用最新导航生效机制，旧流程不会在运行时启动或目录加载后初始化错误会话；侧边栏显示切换目标与失败信息。
- 项目或会话切换完成前会同步锁定侧边栏和标题栏的新建会话入口，避免在半初始化项目上插入空会话。
- 项目重命名和移除改为独占状态事务；删除活动项目时固定替代项目与会话，失败会恢复原项目连接，完成后再开放导航入口。
- 登录、认证刷新和启动流程按请求代次与项目地址隔离；退出登录会使在途认证及会话初始化失效，迟到响应不再恢复旧凭据。
- 产品状态保存改为串行写入并在执行时抓取最新快照；旧保存回包不再覆盖正在编辑的草稿、会话、项目或终端状态。
- 待发送队列随线程持久化，重启后可继续发送；附件仅保存路径并在发送前重新读取、校验，文件失效或能力不匹配时保留队列并明确报错。
- 文件创建、目录创建、重命名和删除绑定发起项目与目录；切换工作区后旧操作不再刷新、清空或报错到新工作区，创建入口会阻止重复提交。
- 文件名搜索结果会按真实类型打开文件或进入目录；新建和重命名前检查当前目录同名项，避免通过写入或移动接口覆盖已有文件，并展示后端返回的具体失败原因；Monaco 编辑器会随 GUI 亮色、暗色或系统主题切换。
- 终端关闭和重连会等待 PTY 释放的真实 HTTP 结果；释放失败时保留 pane 并显示错误，按钮与快捷键共用操作锁，不再静默遗留不可见会话。
- 远程渠道连接、解绑和删除绑定原项目与页面代次，渠道卡片、插件写操作、任务操作和设置编辑器使用同步事务锁，快速重复点击或切换项目后不会重复请求或回写旧页面。
- 快捷键校验、保存和恢复默认使用单一写事务，避免快速重复操作以旧配置覆盖新配置；统计、指标和监控刷新按项目使用同步请求锁，重叠刷新不会提前结束忙碌状态。
- 同一会话的重命名与删除按会话串行执行；侧边栏项目与会话操作、聊天重新连接、停止生成和待发送队列使用同步事务令牌，双击发送不会把同一草稿重复加入队列，切换项目后旧结果不会污染当前聊天。
- 设置页补齐网关认证编辑入口并使用掩码输入，认证值不会写入 GUI 的本地设置缓存；聊天用量显示真正服从“显示 Token 计数”设置。
- “允许剪贴板贴图”现在会在聊天输入框接收 PNG、JPEG、GIF 和 WebP 图片，复用运行时能力与 20MB 限制校验，并保存为可在待发送队列中跨重启恢复的临时附件；90 天前的剪贴板临时图片会自动清理。
- CodeBuddy 提示建议会按当前会话显示在输入框上方，可一键填入草稿或关闭；发送新消息后会清除旧建议，“提示建议”开关现在真正控制界面行为。
- 会话更新会从 CodeBuddy 的 _meta 契约提取提示建议和团队状态；聊天页可查看当前团队成员，并显示此前会被静默丢弃的执行错误、产物、检查点、任务和目标动态。
- 权限在其他客户端处理后会同步关闭对应卡片并保留真实决定；目标进度、目标状态、历史恢复和运行阶段从 CodeBuddy 元数据进入当前会话，避免界面停留在过期的待处理状态。
- 执行 /clear 或 /reset 后，当前 GUI 对话会接管 CodeBuddy 返回的新 sessionId，清理旧上下文和待发送数据，并重新加载新会话配置；加载失败时保留新 ID 和可恢复的明确错误，不再继续向失效会话发送。
- CodeBuddy 的 _codebuddy.ai/question 和 session/request_permission 服务端请求会保留 JSON-RPC 请求 ID 并正确回包；问题卡支持单选、多选、文本答案与取消，标准权限请求和跨客户端权限广播会去重并兼容扩展接口收口。
- 登录状态检查失败不再被误判为已登录，服务不可用时会停留在明确的恢复界面并允许重试，运行中认证失效会返回登录页而不是卡在错误状态；侧边栏、Worker、任务、设置和插件页改为按需订阅状态，流式对话不再触发无关重页面整体重渲染。
- 主进程报告 ACP SSE 超时截断时，渲染层不再把不完整响应视为成功；对话会进入明确错误状态并恢复草稿与附件，方便用户重试。
- ACP 连接更换或会话客户端释放时，旧连接上的 JSON-RPC 权限和问题卡会立即标记失效并取消操作按钮，不再让用户对无法回包的请求继续操作。
- 应用重启、项目运行时更换或异常退出后，终端保留布局和最近输出，但会自动丢弃已失效的 PTY 会话 ID，并在新运行时就绪后重建真实终端。
- 终端尺寸变化按渲染帧合并 PTY resize 请求，日志自动刷新也不再在慢请求期间重复发起后端调用。
- 每个项目会保存上次打开的目录、文件和未保存编辑内容；切换项目或重启应用后会恢复工作现场，明确选择放弃修改时则保存为干净状态。
- 待发送提示队列支持上移、下移和删除，顺序调整与队列内容一起持久化，失败时回滚到操作前状态。

## 使用要求

- Windows x64。
- 本机可直接运行 `codebuddy` CLI，并已完成其必要配置。

## 数据保留

- 覆盖安装会保留现有项目和对话数据。卸载程序默认保留用户数据，方便后续重装恢复。
- 产品状态保留最近一次有效备份；主状态文件损坏时自动恢复备份，并保留原始坏文件用于诊断。
