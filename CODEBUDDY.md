...
GUI 在 `electron/cli-compat.cjs` 中声明最低/推荐 CLI 版本（当前最低 `2.125.0`，推荐 `2.138.0`）。`runtime:ensure` / `runtime:restart` 会在启动前探测版本：低于最低版本、缺失或无法识别时硬阻断，并引导用户在设置页安装推荐版本。高于推荐版本仅警告，不阻断。
